...
- **GATE 4 approval belongs to the human.** After approval: `echo <branch> > .claude/.gate4-ok` → merge → `rm .claude/.gate4-ok`.
...
- `infra-state.md` kept current · needs → `NEEDS-FROM-USER.md` + **STOP** · **hand-in-hand:** "deployed" is never declared by AI alone — verified together in the real environment.
